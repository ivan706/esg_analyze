...
def construct_url_and_header(company_code):
    ...
    data['companyCd'] = data['companyCd'].format(company_code=company_code)
...
def extract_esg_data(data):
    ...
    """
    try:
        companies = data[0]['listInfo']['content']
        return companies
    except Exception as e:
...
def write_esgs_to_file(esgs):
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        ...
        # 数据
        for company in esgs:
            writer.writerow([company['companyCd'], company['companyName'], company['disclosureTitle'], company['publishDate']])


if __name__ == '__main__':
    _companies = []
    with open('listed_companies/bse.csv', 'r') as f:
        reader = csv.reader(f)
        _companies = [(row[0], row[1]) for row in reader]
    # _companies = [('872392', '佳合科技')]
    esgs = []
